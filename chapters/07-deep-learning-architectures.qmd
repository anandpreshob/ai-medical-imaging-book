...
![A CNN builds a hierarchy of representations: local filters produce edge-sensitive feature maps, deeper layers combine them into anatomical and semantic patterns, and the compressed representation drives a prediction.](../figures/ch07/cnn-feature-hierarchy.png){#fig-cnn-feature-hierarchy fig-alt="A chest radiograph passes through a highlighted local filter, stacks of progressively smaller feature maps, deeper feature representations, and a final prediction."}
...
![The U-Net hourglass. The encoder compresses an MRI into deep features, the decoder restores spatial resolution, and skip connections preserve fine boundary information for the final segmentation.](../figures/ch07/unet-encoder-decoder.png){#fig-unet-hourglass fig-alt="A U-shaped encoder-decoder network takes a brain MRI as input and produces a tumor segmentation, with horizontal skip connections joining matching resolution levels."}
...
![A vision transformer divides a fundus image into patches, converts the patches into tokens, and uses self-attention to combine information from distant retinal regions into a global prediction.](../figures/ch07/vision-transformer.png){#fig-vision-transformer fig-alt="A retinal fundus image is divided into a patch grid, converted to a token sequence, passed through a self-attention matrix and global representation, and classified."}
