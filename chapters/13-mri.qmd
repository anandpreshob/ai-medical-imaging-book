...
![Conceptual comparison of MRI sequence families and the information they emphasize. The cards describe acquisition contrasts, not diagnostic rules or patient images.](../figures/ch13/sequences.svg){#fig-mri-sequences fig-alt="Four MRI sequence cards covering T1 anatomy, T2 and FLAIR fluid sensitivity, diffusion and ADC, and susceptibility."}
...
![MRI reconstruction separates measured k-space from an estimated image and downstream interpretation.](../figures/ch13/reconstruction.svg){#fig-mri-reconstruction fig-alt="Measured k-space enters reconstruction with a data-consistency check; the resulting image undergoes image-quality and clinical-task assessment."}
...
![A sequence inventory precedes alignment and multi-sequence inference. Outputs are returned to the original reference grid.](../figures/ch13/alignment.svg){#fig-mri-alignment fig-alt="T1, T2, FLAIR, and diffusion inputs are checked for completeness, aligned to a documented reference, processed, and mapped back to source coordinates."}
...
![A longitudinal comparison requires consistent acquisition, spatial matching, and uncertainty-aware measurement.](../figures/ch13/longitudinal.svg){#fig-mri-longitudinal fig-alt="Baseline and follow-up studies each pass protocol and quality checks before registration, lesion matching, and a reviewed change estimate."}
