...
![The DICOM information hierarchy. One patient can have many imaging studies; each study contains one or more acquisition series; and each series contains ordered DICOM instances, classically one object per slice.](../figures/ch03/dicom-information-hierarchy.png){#fig-dicom-information-hierarchy fig-alt="A four-level diagram labeled Patient, Study, Series, and Instance. A patient leads to one CT imaging event, which contains multiple image series; one selected series expands into many ordered axial CT slice objects, each with a unique identifier."}
...
![The hospital imaging loop. Orders and reports travel between the EHR and departmental systems as HL7/FHIR (dashed teal); images and AI results travel through the scanner, PACS/VNA, AI endpoint, and radiologist workstation as DICOM (solid navy). AI assists from a routed copy and returns results to the archive; the radiologist remains responsible for the signed report.](../figures/ch03/hospital-imaging-workflow.png){#fig-hospital-imaging-workflow fig-alt="A seven-stage workflow: an EHR order goes to RIS scheduling, then CT acquisition, PACS or VNA archiving, optional AI assistance, radiologist reading, and a signed report returned to the EHR. Solid arrows denote DICOM; dashed arrows denote HL7 or FHIR."}
