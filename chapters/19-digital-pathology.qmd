...
![A whole-slide pyramid supports overview navigation and high-resolution regional analysis.](../figures/ch19/pyramid.svg){#fig-path-pyramid fig-alt="Three resolution levels of a slide pyramid lead to selected high-resolution tiles, each retaining its source coordinates and physical scale."}
...
![The specimen hierarchy preserves provenance and keeps correlated material together during evaluation.](../figures/ch19/hierarchy.svg){#fig-path-hierarchy fig-alt="Patient, case, specimen, block, slide, and tile form a nested provenance hierarchy; train-test assignment is made above the tile level."}
...
![Multiple-instance learning aggregates tile representations under a slide-level label.](../figures/ch19/mil.svg){#fig-path-mil fig-alt="Tiles from one slide enter a shared encoder, producing feature vectors that are aggregated into a slide score; attention weights are distinguished from lesion probabilities."}
...
![Quality and stain processing are explicit stages whose effects must be checked against the target task.](../figures/ch19/quality.svg){#fig-path-quality fig-alt="Source-slide quality review precedes optional documented stain processing, tile inference, and review on the original image."}
