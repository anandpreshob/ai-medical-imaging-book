...
![Surgical video tasks operate at different time scales, from one annotated frame through an event and phase to the complete procedure.](../figures/ch18/timescales.png){#fig-video-timescales fig-alt="Laparoscopic cholecystectomy frames illustrate the nested scales of one frame, a clip-application event, a dissection phase, and the entire procedure."}
...
![End-to-end latency can make an otherwise accurate overlay stale relative to the current instrument position.](../figures/ch18/latency.png){#fig-video-latency fig-alt="An operating-room monitor and three laparoscopic frames compare camera capture, the current frame, and a stale displayed instrument overlay."}
...
![Procedure-level splitting keeps every frame, clip, still, and derivative from each operation in one partition.](../figures/ch18/splits.png){#fig-video-splits fig-alt="Three grouped laparoscopic filmstrips place complete procedures and their derivatives in train, validation, or test sets."}
...
![Causal inference uses current and past endoscopic frames; offline analysis may also use future frames.](../figures/ch18/causality.png){#fig-video-causality fig-alt="Seven sequential endoscopic frames show a real-time context window ending at now and an offline window extending into future frames."}
